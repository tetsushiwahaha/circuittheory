...
コースの定義からもPCが主体的に使えることが求められるであろう．
...
もう2.7系は古い．2022.9.13時点では 3.10.6 が最新となっている．常に最新の
環境になるようアップデートする術も確認しておこう．

以下に OS ごとのPython環境の整え方をごく簡単に述べる．
...
有料であったり無償であったりする．情報コースのみなさんは
ソフトウェア実験で，Ubuntu を使うことになるだろう．

Python環境はLinuxにおいてはパッケージで提供されるだろう．
各種インストーラ，apt-get, yumyum などでインストール
...
端末(Terminal.app)を用いるとよい．二つほどPython入手方法を示す．
...
バージョン別に，ソースレベルからインストールできるようになっている．
...
source.py のようにソースコードがすでにファイルとなっている場合は，
```
% python source.py
```
と入力して実行させるとよいだろう．
エラーが発生すれば英語でメッセージが表示されるので，それを読んで根気よく
デバッグする必要がある．これはどのコンピュータ言語でも同様だろう．

2023/10/28 注： macOS Sonoma (14.0) では一部のコードが動きません．そのう
ちフィックスされると思いますが，お待ち下さい．
...
1/2 + j √3/2  の近似値となっている．
なお，πは`np.pi` であり，平方根は`np.sqrt()` で求められるし，三角関数は
`np.sin()`, `np.cos()`, `np.tan()`である．
